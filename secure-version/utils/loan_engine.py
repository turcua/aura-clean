...
Pure, deterministic math — identical in both app versions, not a
vulnerable/secure contrast point. No database access, no AI, no
side effects: everything here is a plain function of its inputs, which is
exactly what lets Actual, Baseline, and future what-if simulations (Sprint
29/31) all be the same function called with different event lists rather
than three separately-maintained pieces of state.
...
Bug fixed 2026-08-24 (BUG-20, deeper root cause than the fix above):
get_payment_impact() diffed two full-lifetime replays — Actual (pinned by
every real snapshot, all the way to payoff) against a per-payment Variant
(pinned only up to that payment's own date, then unpinned pure math the
rest of the way). For an old payment, that meant years of real snapshot
corrections Actual benefited from and Variant never saw, so accumulated
model-vs-reality drift — not the payment itself — dominated the measured
impact, occasionally even flipping its sign negative (confirmed against
real 2023/2025 payments). Recent payments looked fine only because there
was little time for the asymmetry to accumulate. Fixed by adding
start_override (above) so get_payment_impact() can run two *symmetric*
short projections from a shared, already-correct anchor state instead —
both pinned identically (i.e. not at all, after the anchor), so systematic
drift cancels in the diff and only the payment's own presence/absence
survives it. This also made the same-day-snapshot handling (previous fix)
unnecessary for this endpoint — see get_payment_impact()'s own docstring.
"""
...
def project(loan, rate_changes, extra_payments, snapshots, include_extra_payments=True, start_override=None):
    ...
    start_override: optional dict {date, balance, rate_pct, installment} —
                    ...
                    entirely — the override already reflects them. Added
                    2026-08-24 (BUG-20) for get_payment_impact()'s isolated
                    per-payment comparison — see that function's docstring
                    for why replaying full loan history twice per payment
                    was itself the deeper bug, not just same-day ordering.
    ...
    if start_override:
        ...
        # Everything dated in or before the anchor's own period is already
        # reflected in the override's balance/rate/installment — including
        # it again would double-count. Filtering by (year, month), not a
        # bare date comparison, matches extra_by_period's own bucketing
        # below: a payment dated earlier in the anchor's month than the
        # loan's anchor day still belongs to that same period.
    ...
    # tiebreaker for same-day events — without this, two events sharing an
    # effective_date have no defined relative order (DB return order for an
    # exact tie isn't guaranteed), so which one "wins" a same-period pin is
    # luck rather than the real order they were actually logged in. Bug
    # fixed 2026-08-24 (BUG-20) after this surfaced on two same-day bank
    # snapshots in the user's real history; created_at/id already existed
    # on every event row, just weren't threaded through until this fix.
