 ...
 * Ported from vulnerable-version/ Sprint 7 (Phase 2): sidebar collapse controller.
 * Persists collapsed state in localStorage.
 */

// ── Sprint 15: CSRF token injection for every fetch() call ─────
// Every page-specific JS file (categories.js, transactions.js, dashboard.js,
// etc.) calls the global fetch() dozens of times for POST/PUT/DELETE requests.
// Rather than editing every call site to add a header, fetch() itself is
// wrapped once here — this file loads on every page, before any page-specific
// script (see base.html) — so every existing and future fetch() call is
// covered automatically with no per-call-site changes needed. The token
// itself comes from the <meta name="csrf-token"> tag base.html renders using
// Flask-WTF's csrf_token(); GET/HEAD/OPTIONS requests are left untouched since
// CSRFProtect doesn't check those methods either.
(function() {
  ...
  window.fetch = function(input, init) {
    ...
    const needsToken = csrfToken && !['GET', 'HEAD', 'OPTIONS', 'TRACE'].includes(method);
...
// site across the app keeps working unchanged — same option names (icon,
// title, text, timer, showConfirmButton, showCancelButton,
// confirmButtonText, cancelButtonText, didOpen, allowOutsideClick), same
// .then(result => result.isConfirmed) shape.
//
...
// Used by every page-specific script instead of each hardcoding a currency
// symbol. Accounts/transactions carry their own currency (RON/EUR/USD/IDR);
// aggregate totals (net worth, budget balances, dashboard/report widgets)
// are always RON, since MC-005 converts everything to RON before summing.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Sprint 57 (ENH-02, group 6): "Return to Admin" button in the
  // impersonation banner (base.html) — wired here, not admin.js, since
  // the banner (and this file) load on every page, not just /admin.
  const stopBtn = document.getElementById('stopImpersonatingBtn');
  if (stopBtn) {
