...
-- Only the events that aren't already captured elsewhere: extra payments are
-- ordinary `transactions` rows with loan_id set (see below), and on-time
-- payments are assumed per the linked Recurring Transaction, not logged here.
...
SELECT 'Secure Sprint 28 schema created successfully!' AS status;
